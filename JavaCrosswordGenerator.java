...
public class JavaCrosswordGenerator {
    ...
    private final JLabel hintsLabel = new JLabel("Hints: " + INITIAL_HINTS);
    private int score = 0;
    ...
    public JavaCrosswordGenerator(MenuScreen menuScreen) {
        this.menuScreen = menuScreen;
        this.frame = new JFrame("DSA Crossword Generator");
        requestPlayerName();
    }
    ...
    public void setupDictionaryAndClues() {
    ...
    public void loadDefaultCrossword() {
    ...
    public void buildUI() {
        ...
        viewLeaderboardItem.addActionListener(e -> Leaderboard.showLeaderboard(frame));
        
        gameMenu.add(mainMenuItem);
        gameMenu.add(viewLeaderboardItem);
        ...
        revealBtn.addActionListener(e -> {
            int response = JOptionPane.showConfirmDialog(
                frame,
                "Are you sure you want to reveal the answers?\n" +
                "This will end the game and you won't get any score.",
                "Confirm Reveal",
        ...
        level1Btn.addActionListener(e -> loadLevel(1));
        level2Btn.addActionListener(e -> loadLevel(2));
        level3Btn.addActionListener(e -> loadLevel(3));
        level4Btn.addActionListener(e -> loadLevel(4));
        
        undoBtn.addActionListener(e -> undoLastAction());
        redoBtn.addActionListener(e -> redoLastAction());
    ...
    private void checkAllWords() {
        ...
        if (gameEnded) {
            JOptionPane.showMessageDialog(
                frame,
                "The game has ended. You cannot check after revealing answers.",
                "Game Ended",
                JOptionPane.WARNING_MESSAGE
            );
            return;
        ...
        boolean allCorrect = correctWords == placed.size();
        String message = String.format("Correct: %d/%d\nValid: %d/%d\nScore: %d", 
    ...
    private void showHint() {
        ...
        for (Placement p : placed) {
            for (int i = 0; i < p.word.length(); i++) {
                ...
                int cc = (p.dir == Direction.ACROSS) ? p.col + i : p.col;
                String text = cellFields[rr][cc].getText();
                if (text.isEmpty() || text.charAt(0) != p.word.charAt(i)) {
                    candidates.add(new Point(rr, cc));
                }
            }
        }
        
        if (candidates.isEmpty()) {
            JOptionPane.showMessageDialog(frame, "All cells are correct!");
            return;
        }
        
        // Select a random cell to reveal
        Point selected = candidates.get(new Random().nextInt(candidates.size()));
        int row = selected.x;
        int col = selected.y;
        
        // Find the correct letter for this cell
        char correctLetter = solution[row][col];
        
        // Reveal the letter
        cellFields[row][col].setText(String.valueOf(correctLetter));
        cellFields[row][col].setBackground(new Color(200, 255, 200));
        ...
        JOptionPane.showMessageDialog(frame, 
            String.format("Revealed letter '%c' at position [%d,%d]\nHints remaining: %d", 
                correctLetter, row, col, hintsRemaining),
            "Hint Used",
            JOptionPane.INFORMATION_MESSAGE);
    }
